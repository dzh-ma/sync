...
export default function StatisticsPage() {
  ...
  // Fetch data from database backend via our API route
  ...
  const handleExportPDF = async () => {
    try {
      ...
      toast({
        ...
        description: "Your energy report is being generated...",
      });
      
      // Dynamically import libraries to avoid server-side issues
      ...
      if (!container) {
        throw new Error("Statistics container element not found");
      }
      ...
      // Add title and summary to first page
      doc.setFontSize(22);
      doc.text('Energy Consumption Report', 105, 20, { align: 'center' });
      
      doc.setFontSize(14);
      doc.text(`Generated on: ${date}`, 20, 35);
      doc.text(`Time Period: ${timeRangeLabel}`, 20, 45);
      
      // Add summary data
      doc.setFontSize(16);
      doc.text('Summary', 105, 60, { align: 'center' });
      
      doc.setFontSize(12);
      doc.text(`Total Energy Consumed: ${statisticsData.totalEnergyConsumed.toFixed(2)} kWh`, 20, 70);
      doc.text(`Total Cost: ${statisticsData.totalCost.toFixed(2)} AED`, 20, 80);
      doc.text(`Most Active Room: ${statisticsData.mostActiveRoom.name}`, 20, 90);
      doc.text(`Energy Savings: ${statisticsData.energySavings}%`, 20, 100);
      
      // Add device types section
      doc.setFontSize(16);
      doc.text('Device Types', 105, 120, { align: 'center' });
      
      // Add device type data
      let yPos = 130;
      statisticsData.deviceTypeData.forEach((device, index) => {
        if (yPos > 270) {
          doc.addPage();
          yPos = 20;
          doc.setFontSize(16);
          doc.text('Device Types (continued)', 105, yPos, { align: 'center' });
          yPos += 10;
        }
        
        doc.setFontSize(12);
        doc.text(`${device.type}: ${device.consumption.toFixed(2)} kWh (${device.percentage.toFixed(1)}%)`, 20, yPos);
        yPos += 10;
      });
      ...
      doc.setFontSize(16);
      doc.text('Rooms', 105, yPos, { align: 'center' });
      yPos += 10;
      
      // Add room data
      statisticsData.roomData.forEach((room, index) => {
        if (yPos > 270) {
          doc.addPage();
          yPos = 20;
          doc.setFontSize(16);
          doc.text('Rooms (continued)', 105, yPos, { align: 'center' });
          yPos += 10;
        }
        
        doc.setFontSize(12);
        doc.text(`${room.name}: ${room.consumption.toFixed(2)} kWh (${room.percentage.toFixed(1)}%)`, 20, yPos);
        yPos += 10;
      });
      
      // Try to capture the chart elements
      try {
        const charts = container.querySelectorAll('.chart-container');
        if (charts.length > 0) {
          doc.addPage();
          doc.setFontSize(16);
          doc.text('Energy Consumption Visualizations', 105, 20, { align: 'center' });
          
          let offsetY = 30;
          
          // Convert up to 2 charts to images and add to PDF
          for (let i = 0; i < Math.min(charts.length, 2); i++) {
            ...
            doc.addImage(imgData, 'PNG', 20, offsetY, imgWidth, imgHeight);
            offsetY += imgHeight + 10;
            
            if (offsetY > 250 && i < Math.min(charts.length, 2) - 1) {
              doc.addPage();
              offsetY = 20;
            }
          }
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text('SmartHome Energy Report', 20, 290);
      }
      
      // Save the PDF
      doc.save(`energy-report-${timeRange}-${date.replace(/\//g, '-')}.pdf`);
