...
const registerUser = async (email: string, password: string) => {
  ...
  // Generate a stable household ID based on email
  const householdId = `household-${Date.now()}-${email.split('@')[0]}`;
  
  // Store in localStorage for new user
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      name: "Admin",
      role: "Admin",
      householdId: householdId, // Store the household ID
    })
  );
  
  // Also store generated household ID separately for backup
